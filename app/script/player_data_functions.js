...
async function search(
  ...
  xhttp2.open("POST", "/API/search_game", true);
...
function displayStats(json, request) {
  ...
  if (json.whites.length > 0) {
    ...
    for (let i = 0; i < json.whites.length; i++) {
      let tr = document.createElement("tr");
      let td1 = document.createElement("td");
      td1.innerText = json.whites[i].opening;
      let td2 = document.createElement("td");
      ...
      filter.href = `/player_data/?fullname=${encodeURIComponent(
        request.fullname
      )}&color=white&opening=${encodeURIComponent(json.whites[i].opening)}`;
  ...
  if (json.blacks.length > 0) {
    ...
    for (let i = 0; i < json.blacks.length; i++) {
      ...
      filter.href = `/player_data/?fullname=${encodeURIComponent(
        request.fullname
      )}&color=black&opening=${encodeURIComponent(json.blacks[i].opening)}`;
      filter.target = "_self";
      td4.append(filter);
      tr.append(td1);
      tr.append(td2);
...
async function filter(player, color, opening) {
  const xhttp2 = new XMLHttpRequest();
  xhttp2.open("POST", "/API/search_player_opening_game", true);
